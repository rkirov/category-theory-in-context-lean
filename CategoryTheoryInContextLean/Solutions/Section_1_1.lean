...
def Category.maximal_subgroupoid {α : Type*} [Category α] : Subcategory α where
  ...
  id_mem x := by
    unfold Category.IsIso
    simp [id_comp]
  comp_mem _ _ _ hf hg := by
    obtain ⟨f',hf1,hf2⟩ := hf
    obtain ⟨g',hg1,hg2⟩ := hg
    use (g' ≫ f')
    constructor
    · rw [Category.assoc, ← Category.assoc _ g', hg1, id_comp, hf1 ]
    rw [Category.assoc, ← Category.assoc f', hf2, id_comp,hg2]

-- exercise 1.1.i
theorem Category.pair_inverse_iso {X Y : α} (f : Hom X Y) (g : Hom Y X) (h : Hom Y X)
  (hg : f ≫ g = id X) (hf : h ≫ f = id Y) : g = h ∧ IsIso f := by
    have : g = h := by rw [← id_comp g,← hf, assoc, hg, comp_id]
    rw [← this] at hf
    exact ⟨this,⟨g,hg,hf⟩ ⟩
